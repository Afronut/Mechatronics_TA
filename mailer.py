...
p1=subprocess.Popen('iwgetid -r',shell=True,stdout=subprocess.PIPE)
data1=p1.communicate()
# Creates a sentence for each ip address.

my_ip_a = 'Hello partner, your %s ip is %s \nYou are connected to "%s"\nPlease make sure your computer is connected to the same wifi for you to reach me.\nFor a remote access type in a terminal "sudo ssh pi@yourIP"\nif you are asked for password, your default password should be "raspberry".\nHave Question? Do not hesitate to ask for help\nHave a blessed day!\n\nYour partner' % (ip_type_a, ipaddr_a, data1[0])
